...
impl From<std::num::ParseFloatError> for LeosError
{
    fn from(_error: std::num::ParseFloatError) -> Self
    {
        LeosError::ParseError("Error parsing float".to_string())
    }
}

impl From<cast::Error> for LeosError
{
    fn from(error: cast::Error) -> Self
    {
        LeosError::CastError(error)
    }
}

impl From<crate::cocktail::ConversionError> for LeosError
{
    fn from(error: crate::cocktail::ConversionError) -> Self
    {
        LeosError::ConversionError(error)
    }
}
...
impl Leosbar
{
    pub fn from_document(&mut self, document: &Document)-> Result<Self, LeosError>
    {
        let mut garnish = None;
